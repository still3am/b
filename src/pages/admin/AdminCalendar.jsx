...
export default function AdminCalendar() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-cormorant text-4xl font-bold text-foreground">Calendar Management</h1>
        <p className="font-dm text-muted-foreground text-sm mt-1">Set availability, block days, customize time slots.</p>
      </div>
        ...
        <div className="glass-card rounded-3xl p-6">
          <div className="flex items-center justify-between mb-5">
            <button onClick={() => { if(month===0){setYear(y=>y-1);setMonth(11);}else setMonth(m=>m-1); setSelectedDay(null); }}
              className="w-9 h-9 rounded-full glass-card flex items-center justify-center text-pink-500 hover:border-pink-300 transition-all">
              <ChevronLeft size={16} />
            </button>
            <h2 className="font-cormorant text-xl font-semibold">{monthName}</h2>
          ...
          <div className="grid grid-cols-7 mb-2">
            {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => (
              <div key={d} className="text-center text-xs font-dm font-medium text-muted-foreground py-1">{d}</div>
            ))}
          </div>
            ...
            {Array(daysInMonth).fill(null).map((_,i) => {
              ...
              return (
                <button key={day} onClick={() => status !== 'sunday' && selectDay(day)}
                  className={`aspect-square rounded-xl flex items-center justify-center text-sm font-dm font-medium transition-all border border-transparent ${
                    isSelected ? 'chrome-button' : `${dayColors[status]}`
          ...
          <div className="flex flex-wrap gap-3 mt-4 text-xs font-dm">
            {[
          ...
          {!selectedDay ? (
            <div className="h-full flex items-center justify-center text-center min-h-[300px]">
              <div>
                ...
                <p className="font-dm text-muted-foreground text-sm">Click a day to edit its availability.</p>
              </div>
            </div>
          ) : (
            <motion.div key={selectedDay} initial={{ opacity:0, x:10 }} animate={{ opacity:1, x:0 }} className="space-y-5">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsBlocked(b => !b)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-dm font-medium transition-all ${isBlocked ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}>
              ...
              {isBlocked ? (
                <div>
                  <label className="font-dm text-xs font-medium text-muted-foreground mb-1 block">Block Reason (optional)</label>
                  <input value={blockReason} onChange={e => setBlockReason(e.target.value)}
                    placeholder="e.g. Holiday, Staff unavailable..." className="input-field" />
                </div>
              ) : (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="font-dm text-xs font-medium text-muted-foreground">Toggle time slots</p>
                    <div className="flex gap-2">
                      <button onClick={() => setEditSlots(DEFAULT_SLOTS)} className="text-xs font-dm text-pink-500 hover:text-pink-700">All</button>
                      <button onClick={() => setEditSlots([])} className="text-xs font-dm text-muted-foreground hover:text-foreground">None</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-4 gap-1.5">
                    {DEFAULT_SLOTS.map(slot => (
                      <button key={slot} onClick={() => toggleSlot(slot)}
                        className={`py-2 rounded-lg text-xs font-dm font-medium transition-all ${
                          editSlots.includes(slot) ? 'chrome-button' : 'glass-card text-pink-300 hover:border-pink-200'
                        }`}>
                        {slot}
                      </button>
                    ))}
              ...
              {dayBookings.length > 0 && (
                <div>
                  <p className="font-dm text-xs font-medium text-muted-foreground mb-2">Bookings on this day ({dayBookings.length})</p>
                  <div className="space-y-1.5">
                    {dayBookings.map(b => (
                      <div key={b.id} className="flex items-center gap-2 text-xs bg-pink-50 rounded-lg px-3 py-2">
                        <span className="font-dm font-medium text-foreground">{b.booking_time}</span>
                        <span className="font-dm text-muted-foreground">{b.customer_name} — {b.service_name}</span>
                        <span className={`ml-auto font-dm ${b.status==='confirmed' ? 'text-green-600' : 'text-yellow-600'}`}>{b.status}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <button onClick={saveAvailability} disabled={saving}
                className="w-full chrome-button font-dm font-medium py-2.5 rounded-full text-sm flex items-center justify-center gap-2 disabled:opacity-50">
